...
declare -a SYS_TOOLS=("mkdir" "dirname" "date" "touch" "wget" "mount" "unzip")
[[ "$HOST_OS" = Darwin ]] && declare -a SYS_TOOLS+=("shasum") || declare -a SYS_TOOLS+=("sha256sum")
...
cat <<_EOF
  Usage: $(basename "$0") [options]
    OPTIONS:
      ...
      --no-radio   : [OPTIONAL] Don't copy radio images (default: false)
...
needs_oatdump_update() {
  ...
  deps_cur_sig=$(shasum256 "$deps_zip" | cut -d ' ' -f1)
  if [[ "$HOST_OS" == "Darwin" ]]; then
